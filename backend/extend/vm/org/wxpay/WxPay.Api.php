...
class WxPayApi
{
     ...
     */
    public static function transfer(WxPayConfigInterface $config, $tradeNo, $openid, $amount, $desc = '', $name = '', $timeOut = 6)
    {
        ...
        $data = array(
            ...
            'partner_trade_no' => $tradeNo,//商户订单号
            'openid' => $openid,//用户openid
            'check_name' => empty($name) ? 'NO_CHECK' : 'FORCE_CHECK',//校验用户姓名选项,
        ...
        foreach ($data as $k => $v)
        {
            if($k != "sign" && !empty($v) && !is_array($v)){
                $buff .= $k . "=" . $v . "&";
            }
        }
        $buff = trim($buff, "&");
        $buff .= '&key=' . $config->GetKey();
...
//        $result = WxPayResults::Init($config, $response);
    ...
    public static function queryTransferInfo(WxPayConfigInterface $config, $tradeNo)
    {
        $data = array(
            'appid' => $config->GetAppId(),//商户账号appid
            'mch_id' => $config->GetMerchantId(),//商户号
            'nonce_str' => self::getNonceStr(),//随机字符串
            'partner_trade_no' => $tradeNo,//商户订单号
        );
        ...
        //生成签名算法
        $xml = arrayToXml($data);
        $url = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/gettransferinfo'; //调用接口

        $startTimeStamp = self::getMillisecond();//请求开始时间
        $response = self::postXmlCurl($config, $xml, $url, true, 6);
//        $result = WxPayResults::Init($config, $response);
        $result = xmlToArray($response);
        self::reportCostTime($config, $url, $startTimeStamp, $result);//上报请求花费时间
        return $result;
